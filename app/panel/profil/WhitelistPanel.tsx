...
export type TabId = "profil" | "questionnaire" | "personnage";

export default function WhitelistPanel() {
  const { data: session, status: sessionStatus } = useSession();
  const router = useRouter();
  const searchParams = useSearchParams();
  const tab = (searchParams.get("tab") as TabId | null) ?? "profil";
  ...
  const goTo = useCallback(
    (next: TabId) => {
      router.push(`/panel/profil?tab=${next}`);
    },
    [router]
  );
  ...
  if (
    ...
    (sessionStatus === "authenticated" && application === undefined)
  ) {
    return (
  ...
  if (!session?.user?.inGuild) {
    return (
      <div className={styles.card}>
        ...
        <p className={styles.cardText} style={{ marginBottom: 24 }}>
          Tu dois être membre du serveur Discord Epsilon Roleplay pour
          candidater à la whitelist.
        </p>
  ...
  const profileComplete =
    application?.playtime_hours != null && application?.age != null;
  ...
  return (
    <div>
      {submitted && (
        <StatusBanner application={application as Application} />
      )}
