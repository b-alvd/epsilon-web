...
export default function QuestionnaireTab({
  ...
  const isSubmitted = appStatus !== undefined && appStatus !== "draft";

  if (alreadyPassed || (isSubmitted && appStatus !== "rejected_quiz")) {
    return (
      <div className={styles.card}>
  ...
  if (attemptsExhausted || appStatus === "rejected_quiz") {
    return (
      <div className={styles.card}>
  ...
  async function startQuiz() {
    ...
    if (!res.ok || !Array.isArray(data.questions) || data.questions.length === 0) {
      console.error("[quiz] GET failed:", data);
      setError(`Impossible de charger les questions (${data.error ?? "unknown"}).`);
      return;
    }
  ...
  if (!started) {
    return (
      <div className={`${styles.card} ${styles.cardNarrow}`}>
        ...
        <div className={styles.actions}>
          <button type="button" className={styles.button} disabled={loading} onClick={startQuiz}>
